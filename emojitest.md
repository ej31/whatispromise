...
```mermaid
sequenceDiagram
    participant MN as 메인 닌자 :ninja:
    participant CS as 분신: fetchData :ninja:
    participant NW as 네트워크
    participant JS as JSON 파싱

    MN->>+CS: 분신술 발동 (fetchData 호출) :ninja:
    Note over CS: 분신이 요청을 준비함 :ninja:
    CS->>+NW: 은밀한 데이터 요청 (fetch) 
    Note over NW: 네트워크가 응답을 처리함 
    NW-->>-CS: 응답 도착
    Note over CS: 분신이 데이터를 파싱 :ninja:
    CS->>+JS: 응답을 JSON으로 변환
    JS-->>-CS: JSON 데이터 준비 완료
    CS-->>-MN: 데이터 반환
    Note over MN: 메인 닌자가 결과를 검토 :ninja:
